...
vi p; // meaning: parent
int s = 5; // starting vertex (or source)
...
int main() {
	...
	for (int i = 0; i < V; i++) { // Printing adjList
		cout << i << " is connected to: ";
		for (int j = 0; j < adjList[i].size(); j++)
		// For all the vertices that are connected to vertex i
			cout << adjList[i][j] << " ";
		if (!adjList[i].size()) cout << "NONE";
		cout << endl;
	}
	...
	printf("\n\nBelow are minimum weights required to travel from vertex %d (source) to other vertices:\n", s);
	for (int i = 0; i < V; i++)
	...
	cout << endl;
	printPath(3); // prints shortest path from vertex 5 (or source) to vertex 3 in correct order
	printf("\nisBipartite? %d\n", isBipartite);
